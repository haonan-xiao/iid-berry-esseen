...
from execution_records import (atomic_write_bytes, sha256_file, verify_snapshot,
...
def stage_literal_inputs(snapshot: Path, project: Path, destination: Path) -> None:
    from verify_archive import literal_inputs
...
class Replay:
    ...
    def cutover(self) -> None:
        ...
        from verify_archive import verify_evidence
        verify_evidence(self.snapshot, self.build)
        from verify_manifest import EXPECTED_TASK_PAIRS
    ...
    def finalize(self) -> None:
        from verify_archive import execution_repair_paths
        ...
        from verify_archive import verify_evidence, artifact_paths
